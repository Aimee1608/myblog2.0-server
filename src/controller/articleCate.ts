...
class articleCateController {
  ...
  static async getList(ctx:Context) {
    ...
    // console.log('88888');
    // 查询
    const result = await ArticleCate.paginate({}, options).catch(() => {
  ...
  static async getAllList(ctx:Context) {
    const result = await ArticleCate.find().catch(() => {
      throw new CustomError(500, '服务器内部错误')
    })
    if (result) {
      ctx.data({
  ...
  static async add(ctx:Context) {
    // es6对象解构赋值
    const { name } = ctx.request.body // 请求参数放在请求体
  ...
  static async edit(ctx:Context) {
    const { _id, name, state } = ctx.request.body

    if (!_id) {
      throw new CustomError(500, '无效参数')
    }

    const result = await ArticleCate.findByIdAndUpdate(_id, {
      name,
      state
    }) // new: true ？？？
      .catch(() => {
  ...
  static async delete(ctx:Context) {
    ...
    const result = await ArticleCate.findByIdAndRemove(_id).catch(() => {
      throw new CustomError(500, '服务器内部错误')
    })
    if (result) {
      ctx.data({
